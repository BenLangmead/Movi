...
- [x] **`lookup` canonical ids — correctness DONE & VALIDATED.** In canonical mode `--kmer --kmer-bv` now returns `id = rank(min(lb_fw, lb_rc))`. **Bijection test (ecoli10 k=31, num_kmers=15,209,176): max id 15,209,173 < num_kmers, 0 out-of-range, and the forward-reads vs reverse-complement-reads id multisets are IDENTICAL ⇒ `lookup(x)==lookup(rc(x))`.** This is the SSHash-compatible canonical id space, queryable. (Correctness path searches rc(x) explicitly: ~2 searches/k-mer; the bidirectional fast path below removes the 2nd search.) Still TODO on this line: emit `orientation` in the output; wire onto the bidirectional walk for speed:
- [ ] `lookup` **speed** (bidirectional fast path) + orientation output. **Mechanism (resolves the canonicalize-vs-overlap tension):** the k-mer search already maintains a `MoveBiInterval` with BOTH `fw_interval` (x) and `rc_interval` (rc(x)). Since BWT intervals are ordered lexicographically, `x` is canonical ⟺ `lb_fw ≤ lb_rc`. So **canonical id = `kmerbv_rank1(min(lb_fw, lb_rc))`** and **orientation = forward iff `lb_fw ≤ lb_rc`** — no string canonicalization, no second search, and it stays on the fast *bidirectional keep-going* walk (`query_kmers_from_bidirectional`), closing the speed gap too. Implementation: emit a per-k-mer id from each found `bi_interval` (including the `partial_matches` k-mers); needs `kmerbv_all_p` for both fw and rc lb. VERIFY empirically that `rc_interval` is rc(x)'s forward interval (so its lb maps into B_k) via the bijection test below.
...
- [ ] **TODO (not yet — deferred per BL):** eliminate `kmerbv_all_p` (~131 MB RSS at query) by making the id rank run-local, as was done for the count path. RSS-only (not on disk), so it doesn't affect size-competitiveness; pursue after the lookup/access work.
