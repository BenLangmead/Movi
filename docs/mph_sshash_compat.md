...
- [x] **sd_vector or run-local is the id rep; dense is opt-in** (`build-kmerbv --id-bv-dense`). Default build writes `kmerbv.<k>.sd` plus the run-local `kmerbv.<k>.id.*`; load prefers run-local when present, `MOVI_ID_BV=sd` and `MOVI_ID_BV=dense` select the absolute-row reps. (validated: ecoli10 default build writes only `.sd`, query loads it.)
...
- [x] **`lookup` keep-going id walk — DONE & DEFAULT.** `query_kmers_id_bv` mirrors the count-bv positive-skip walk but emits the canonical MPHF id per k-mer using only `B_k`+`all_p` (no count structure): for a present k-mer the walk's subset interval gives `id = rank_B(lb_row+1)-1` (x's mark is the only `B_k` 1 in its group); non-canonical k-mers use the lazy ftab rc search. **Result: 1.61 M/s vs 0.73 M/s per-k-mer lazy-rc = +120% (2.2×)** (ecoli100 k31, 60 M kmers), **byte-identical ids** (validated vs the per-k-mer path). Now the default `--kmer --kmer-bv` path (no env gate). Note: it emits `pos:presence(1):id[:f|r]`; occurrence counts remain on `--kmer-count --kmer-bv`. Speed summary (canonical MPHF id): 0.48 (always-rc) → 0.75 (lazy-rc) → **1.61 (keep-going)** M/s; ref presence 10, count-bv 2.9, non-canonical-id 1.0. **A bidirectional keep-going walk is the wrong shape for this.** The forward walk holds interval(M) for M = read[pos..anchor], and the k-mer x it resolves is a prefix of M, which is why interval(M) sits inside interval(x). rc(x) is a suffix of rc(M), and interval(rc(M)) is not contained in interval(rc(x)), so a maintained rc interval only lands in the right group for the rightmost k-mer of a run. What works instead is a second unidirectional keep-going walk over rc(read), joined to the forward walk by position: with R' = rc(read) of length L, the walk on R' holds interval(M') for M' = R'[p..a'], R'[p..p+k-1] is a prefix of M' and equals rc(x) for the forward k-mer x at position L-p-k, so the second walk delivers a row inside interval(rc(x)) at one amortised LF step per k-mer instead of the k - ftab_k of a fresh rc search. occ(s) = occ(rc(s)) on the doubled text, so the two walks report the same present k-mer set and pair up position by position. Canonicality still comes from the k-mer itself (packed x vs packed rc(x), maintained incrementally), and the row order confirms it: intervals of distinct equal-length patterns are disjoint and ordered lexicographically, so lb(x) < lb(rc(x)) exactly when x < rc(x). VERIFY rc semantics via the bijection test (already passing on the correctness path, which confirms the `rank(min(lb))` math).
...
- [x] **Run-local rank over `B_k` for the id (`kmerbv.<k>.id.*`), DEFAULT when present.** `id = rank_{B_k}(lb) - 1` is a global prefix count, so unlike the count structure's predecessor/successor it needs a prefix term as well as a within-run one: `id = (marks in runs < run) + (marks in run at offset <= off) - 1`. Marks are split by position inside their run. A mark on a run head is a bit in `id.hd` (rank support built at load), so head marks in earlier runs are one rank. A mark at a positive offset gets an entry in `id.gioffdac` (within-run offsets, delta-coded per run, `dac_vector<2>`), and `id.sep` is a separator vector of length `r + I` in which run `i` contributes a `1` followed by one `0` per off-head mark, so `select_1(i+1) - i` is the number of off-head marks in earlier runs and the zeros after that position, read from the same word, give how many run `i` holds. The rank and the select are independent of each other, and everything is keyed by `run` or by a mark-ordered entry index, so the id path forms no absolute BWT row and builds no `kmerbv_allp_ckpt`. `MOVI_ID_BV=sd` (or an index without `id.*` files) falls back to the sd_vector. **ecoli1 k=31: ids byte-identical to the sd path on ecoli10.query.fa and on the whole key set (5,457,362 rows), bijection test still PASS. On disk 2,706,073 B (hd 1,042,840 + sep 1,194,968 + gioffdac 468,265) vs sd 2,802,244 B, and the sampled checkpoint table (~1.5 MB RSS) disappears. Post-load query 0.145 s -> 0.137 s. Isolating the lookup by stubbing it out: the walk plus the reverse complement walk plus canonicalisation is 0.121 s of that, so the id lookup is 0.026 s with sd and 0.017 s run-local; the second (rc) walk canonical mode requires, not the rank, is what dominates the identifier query.**
...
- [x] **`kmerbv_all_p` SHRUNK via sampled checkpoints (Suggestion B) — DONE, CURRENT.** `B_k` addresses ABSOLUTE BWT rows (universe `n`, the smallest on-disk sd), and the per-run base `all_p[run]` is recovered at query from sparse checkpoints — one absolute position every `kmerbv_allp_S` runs (default 16, `MOVI_ALLP_SAMPLE`-overridable) plus a bounded walk summing the move structure's run lengths (`get_n`, already in RAM): `reconstruct_allp(run) = ckpt[run/S] + Σ_{j=S⌊run/S⌋}^{run-1} get_n(j)`. The walk is paid **once per present k-mer**, not per LF step. Checkpoints are built at load (no format change); `--kmer-count` is run-local and builds none. **Result (ecoli100 k31): vs full all_p, query RSS 576 → 427 MB (−149 MB = 95% of the 157 MB all_p table) at +3% time (S=16); S=64 → 419 MB at +6%. Reconstruction is EXACT — ids byte-identical across S=1/16/64 and sd==dense (validated on ecoli10, 50k reads).** On disk the id sd is back to **32 MB** (0.57× SSHash's 56 MB; the stride rep had inflated it to 62 MB). This **dominates the prior stride approach**: same RSS win, but half the on-disk size and no compile-time `L`/mode coupling. ecoli10: full all_p ≈ 64 MB, S=8 reclaims 56 MB (88%) at ~0% time, S=16 reclaims 60 MB (94%) at +1%. *Superseded: stride addressing (`run*L+offset`, universe `r*L`) — eliminated all_p entirely but inflated the on-disk sd to 62 MB and coupled `L` to the compile-time `MAX_RUN_LENGTH`; sampling keeps the 32 MB sd and is mode-agnostic.*
