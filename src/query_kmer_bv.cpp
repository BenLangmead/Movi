...
void MoveStructure::build_kmerbv(const std::vector<uint32_t>& ks) {
    ...
    for (uint32_t k : ks) {
        ...
        INFO_MSG("k=" + ks_ + ": num_kmers=" + std::to_string(ones) + " distinct " +
                 ...
                 std::to_string(sz_sd / 1048576) + "MB; k-mer files at " +
...
void MoveStructure::load_kmerbv(uint32_t k) {
    ...
    // MPHF-id B_k: the sparse Elias-Fano sd_vector behind the KmerIdBv alias.
    ...
    kmerbv_sd_rank = movi::KmerIdBvRank(&kmerbv_sd);
    kmerbv_sd_sel  = movi::KmerIdBvSel(&kmerbv_sd);
