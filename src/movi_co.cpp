...
// Coroutine return type for query_pml_colh
struct MoveStructure::query_pml_colh_return_type {
    struct promise_type {
        query_pml_colh_return_type get_return_object() {
            return query_pml_colh_return_type{coroutine_handle<promise_type>::from_promise(*this)};
        }
    ...
    query_pml_colh_return_type() : coro(nullptr) {}
    query_pml_colh_return_type(coroutine_handle<promise_type> h) : coro(h) {}
    
    // Copy constructor - delete it since coroutine handles shouldn't be copied
    query_pml_colh_return_type(const query_pml_colh_return_type& other) = delete;
    
    // Move constructor
    query_pml_colh_return_type(query_pml_colh_return_type&& other) noexcept 
        : coro(other.coro) {
        other.coro = nullptr;
    }
    
    // Copy assignment - delete it
    query_pml_colh_return_type& operator=(const query_pml_colh_return_type& other) = delete;
    
    // Move assignment
    query_pml_colh_return_type& operator=(query_pml_colh_return_type&& other) noexcept {
        if (this != &other) {
            if (coro) {
                if (!coro.done()) {
                    coro.destroy();
...
MoveStructure::query_pml_colh_return_type MoveStructure::query_pml_colh(
    SharedFastqReader& reader, 
    ...
    while (true) {
        ...
        while (roff > -1) {
            ...
            if (!check_alphabet(R[roff])) {  // char doens't exist in reference
                ...
                bool up = false;
                if (idx == end_bwt_idx) {
                    // end_bwt_idx_thresholds is indexed by the separator-adjusted alphabet
                    // position (matching production reposition_thresholds), so apply the same
                    // sep_adjust used by the other threshold lookups below.
                    up = offset < end_bwt_idx_thresholds[r_ch_id - sep_adjust];
                    if (up) { // reposition up
                        if (saved_idx == 0) {
                            idx = r;
                        } else {
...
MoveStructure::query_pml_colh_return_type MoveStructure::query_kmer_colh(
    SharedFastqReader& reader,
    coroutine_handle<>& my_handle_storage,
    int coroutine_id) {
...
void process_fastq(const string& fastq_file, const string& index_dir, int concurrency) {
    ...
    std::vector<MoveStructure::query_pml_colh_return_type> coroutines;
