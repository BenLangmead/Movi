...
bool parse_command(int argc, char** argv, MoviOptions& movi_options, bool supress_messages) {
    ...
    auto buildKmerBvOptions = options.add_options("build-kmerbv")
        ...
        ("id-bv-dense", "Also write the dense bit_vector+rank id rep (~6% faster query, ~4-5x larger); default writes only the sparse sd_vector");
        ...
        if (result.count("command")) {
            ...
            if (command == "build") {
                ...
                if (result.count("index") == 1 and result.count("kmer-lengths") >= 1) {
                    ...
                    if (result.count("id-bv-dense") >= 1) { movi_options.set_kmerbv_id_dense(true); }
                } else {
                ...
                if (result.count("index") == 1 and result.count("read") == 1) {
                    ...
                    if (result.count("coroutine") == 1) {
                        movi_options.set_coroutine(true);
                    }
