...
/****  Beginning of functions implemented in move_structure_query.cpp   ****/
        ...
        // Both share one return type and the scheduler scaffolding.
        ...
        query_pml_colh_return_type query_kmer_colh(
            SharedFastqReader& reader,
            std::coroutine_handle<>& my_handle_storage,
            int coroutine_id);
...
/***************************************************************************/
    ...
    private:
        ...
        // K-mer boundary bitvector B_k (the MPHF id structure; rank(lb) gives the
        // id). B_k is sparse -- only canonical k-mers are marked -- so its
        // representation is an Elias-Fano sd_vector behind the compile-time
        // KmerIdBv alias (see kmer_bv_rep.hpp), several times smaller than a dense
        // bit_vector + rank. Built by build_kmerbv(); queried by the --kmer-bv id
        // path. rank/select are built in.
        movi::KmerIdBv     kmerbv_sd;
        movi::KmerIdBvRank kmerbv_sd_rank;
        movi::KmerIdBvSel  kmerbv_sd_sel;
